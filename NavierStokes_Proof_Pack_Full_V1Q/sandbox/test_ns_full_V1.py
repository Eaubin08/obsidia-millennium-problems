...
def run(args):
    ...
    T = args.T; t = 0.0
    rows = []
    for step in range(max(2, args.steps)):
        dt = cfl_dt(u, v, dx, dy, cfl=max(1e-3, args.cfl), nu=nu)
        if t+dt > T: dt = T-t
    ...
    params = {"res":args.res,"nu":args.nu,"T":args.T,"steps":max(2, args.steps),"cfl":max(1e-3, args.cfl)}
